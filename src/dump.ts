...
import type { BigCraftable, Boots, Clothing, Furniture, FurnitureType, Hat, Object, Tool, Weapon } from './types/items/1.6.js';

const objects = JSON.parse(await readFile('./content/Data/Objects.json', 'utf-8')) as Record<string, Object>;
...
const bigCraftables = JSON.parse(await readFile('./content/Data/BigCraftables.json', 'utf-8')) as Record<string, BigCraftable>;
const bigCraftablesArray = Object.values(bigCraftables).map(obj => ({ ...obj, _type: 'BigCraftable' }));
...
const shirts = JSON.parse(await readFile('./content/Data/Shirts.json', 'utf-8')) as Record<string, Clothing>;
const pants = JSON.parse(await readFile('./content/Data/Pants.json', 'utf-8')) as Record<string, Clothing>;
const shirtsArray = Object.values(shirts).map((shirt, i) => ({ ...shirt, _type: 'Shirt', ParentSheetIndex: i, Type: 'Shirt' }));
const pantsArray = Object.values(pants).map((pant, i) => ({ ...pant, _type: 'Pants', ParentSheetIndex: i, Type: 'Pants' }));
...
const weapons = JSON.parse(await readFile('./content/Data/Weapons.json', 'utf-8')) as Record<string, Weapon>;
const weaponsArray = Object.values(weapons).map(weapon => ({ ...weapon, _type: 'Weapon' }));
const tools = JSON.parse(await readFile('./content/Data/Tools.json', 'utf-8')) as Record<string, Tool>;
const toolsArray = Object.values(tools).map(tool => ({ ...tool, _type: 'Tool' }));
...
const writeToFile = JSON.stringify([...objectsArray, ...bigCraftablesArray, ...bootsArray, ...clothingArray, ...furnitureArray, ...hatsArray, ...weaponsArray, ...toolsArray].filter((o) => o).map(item => [item.Name, item]));
