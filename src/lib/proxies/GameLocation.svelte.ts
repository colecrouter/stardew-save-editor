...
import { createItemProxy } from "$lib/proxies/items";
import type { ItemProxy } from "$lib/proxies/items/ItemProxy";
import type {
...
export class GameLocation implements DataProxy<Location> {
	...
	private getItems() {
		...
		const items = Array.from<ItemProxy | null>({ length: height }).map(() =>
			Array.from<ItemProxy | null>({ length: width }).fill(null),
		);
