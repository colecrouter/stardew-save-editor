...
import type { ClothesType } from "$types/save";
import { ItemProxy, Raw } from "./ItemProxy";
import type { ClothingItemModel } from "./types";

const CLOTHING_SLOTS: ParentIndex[] = ["shirtItem", "pantsItem"];

export class ClothingProxy extends ItemProxy {
	constructor(raw: ClothingItemModel) {
		super(raw);
		this.ensureClothing();
	}
